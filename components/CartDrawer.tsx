      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          {items.length === 0 ? (
            ...
            items.map((item, idx) => (
              <div key={item.cartId} className="flex gap-5 animate-fade-in-up">
                <div className="w-24 h-28 bg-[#EBE7DE] flex-shrink-0">
                  <img src={item.imageUrl} alt={item.name} className="w-full h-full object-cover" />
                </div>
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start mb-1">
                        <h3 className="font-serif text-lg text-[#2C2A26]">{item.name}</h3>
                    ...
                    {item.selectedOptions && Object.entries(item.selectedOptions).length > 0 && (
                      <div className="flex flex-wrap gap-x-3 gap-y-1 mb-2">
                        {Object.entries(item.selectedOptions).map(([key, val]) => (
                          <span key={key} className="text-[10px] uppercase tracking-widest text-[#A8A29E]">
                            {key}: <span className="text-[#5D5A53]">{val}</span>
                          </span>
                        ))}
                      </div>
                    )}
                  ...
                  <button 
                    ...
                    className="text-xs text-[#A8A29E] hover:text-red-800 self-start underline underline-offset-4 transition-colors"
                  >
                    Remove
                  </button>
                </div>
        ...
        <div className="p-6 border-t border-[#D6D1C7] bg-[#EBE7DE]/30">
          ...
          <button 
            ...
            className="w-full py-4 bg-[#2C2A26] text-[#F5F2EB] uppercase tracking-widest text-sm font-medium hover:bg-[#433E38] transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-inner"
          >
            Checkout
          </button>
        </div>
