...
const Assistant: React.FC<AssistantProps> = ({ products, onSync, lastSync }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: 'Welcome to Elysoir. I am your private concierge. How may I assist you with our jewelry and timepieces today?', timestamp: Date.now() }
  ]);
        ...
        <div className="bg-[#F5F2EB] rounded-none shadow-2xl shadow-[#2C2A26]/10 w-[90vw] sm:w-[380px] h-[550px] mb-6 flex flex-col overflow-hidden border border-[#D6D1C7] animate-slide-up-fade">
          ...
          <div className="bg-[#EBE7DE] p-5 border-b border-[#D6D1C7] flex justify-between items-center">
            <div className="flex flex-col">
                ...
                {lastSync && (
                    <span className="text-[10px] text-[#A8A29E] uppercase tracking-widest mt-1">
                        Synced: {lastSync}
                    </span>
                )}
            </div>
            ...
            <div className="flex items-center gap-4">
              <button 
                ...
                className={`text-[#A8A29E] hover:text-[#2C2A26] transition-all ${isSyncing ? 'animate-spin' : ''}`}
                title="Sync with Elysoir Boutique"
              >
